...
(function () {
  ...
  if (registration) registration.addEventListener('submit', async event => {
  ...
  function render() {
    const session = read(SESSION, null), user = session && read(USERS, {})[session.email]; if (auth) auth.hidden = Boolean(user); if (cabinet) cabinet.hidden = !user; if (!user) return; if (!cabinet) { location.href = 'account.html'; return; }
